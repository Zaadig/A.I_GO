...
class myPlayer(PlayerInterface):
    def __init__(self):
        self._board = Board()
        self._mycolor = None
        self.model = load_model('my_op_model.h5')

    def getPlayerName(self):
        return "Simo6"

    def get_state_for_nn(self, board):
    ...
    def getPlayerMove(self):
        ...
        # Preparing the current state for the neural network
        nn_input = self.get_state_for_nn(self._board)

        # Predicting the best move using the neural network
        nn_output = self.model.predict(np.expand_dims(nn_input, axis=0))
    ...
    def find_best_move_nn(self, legal_moves):
        ...
        for move in legal_moves:
            score = predicted_moves[self._board.flatten(move) // 2]
